...
- **渲染**：Nyan Pig（`nyanpig.rs` 编译期拼接 `nyanpig-head.html`、`nyanpig.css`、`nyanpig-body.html`、`nyanpig-i18n.js`、`nyanpig.js`、`nyanpig-tail.html`，运行时仍为整页 HTML），`#pet.mood-*` CSS 类驱动配色与动画。
...
- **Success**：`activity == "agent_success"` 时进入 **Success**，内部 **`success_until` 维持 `SUCCESS_HOLD_MS`（4500 ms）**——与 `pet_core.rs` 及 `nyanpig.js` 中 `CELEBRATION_ATTR_HOLD_MS` 一致；到期后落回 Idle 时另有 **`success_to_idle_confirm`（连续 2 帧、约 300ms）**，缓和 Hook 与 Web 侧一帧抖动。
- **Sleeping**：若当前将落在 **Idle**，且距 **`last_activity` 已超过 300 秒**，则改为 **Sleeping**（5 分钟无「活跃」）。
- **活跃**（用于 Sleeping 计时）：新鲜 Hook 上 `activity != "idle"`，或 **`in_flight_tools` 非空**，或 **`subagent_depth > 0`** 时刷新 `last_activity`（见 `pet_core.rs`）。**用户戳猪**（WebView 自定义协议 `poke` → `PetBrain::note_user_poke`）也会刷新 `last_activity`，利于从 **Sleeping** 回到非睡眠计时路径。
...
2. **Success**：当 `activity == "agent_success"` 时，启动 **`SUCCESS_HOLD_MS`（4500 ms）** 的 `success_until`，期间 mood 固定为 **Success**。
...
6. **从 AI 忙碌态回到 Idle**：需连续 **2 次** tick（约 300ms）确认，避免抖动（`idle_enter_confirm`）。
7. **从 Success 回到 Idle**：需连续 **2 次** tick 才允许 Success→Idle（`success_to_idle_confirm`），避免与 `agent_success` / 会话字段一帧抖动叠成闪屏。
...
约 17.5s  success_until 到期（4.5s）       → idle                 Idle（再经 success_to_idle_confirm）
```
