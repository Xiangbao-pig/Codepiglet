...
1. **Phase 1 是否纳入 `focus_file`（仅 basename）**？若纳入，字段从哪个 Hook 事件取（建议 `afterFileEdit` / `preToolUse(Write)` 的 path 字段）？  
2. **UserCoding**：实现 `mood-coding` 还是从文档删除？  
3. **Windows 优先级**：Phase 2 是否与你常用平台同步交付（macOS + Win 同发，还是先 macOS）？

---

*定稿后开发以本文为 sprint 边界；细节变更记在 CHANGELOG 或 PR 描述中。*
